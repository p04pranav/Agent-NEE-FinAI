...
We executed a full backtest on NVIDIA Tesla T4 16GB hardware (CUDA 13.0) using
phi3:mini (3.8B parameters, Q4_0 quantization). The backtest processed 490
predictions across all 10 NSE tickers, sampling every 20th row from 1000-row
synthetic OHLCV CSVs (rows 30–999, indicator warmup excluded).

| Metric | Target | Measured | Status |
|--------|--------|----------|--------|
| Directional accuracy | 62%+ | 33.9% | Below target |
| HIGH confidence accuracy | 70%+ | 26.1% (n=46) | Below target |
| MED confidence accuracy | 55–65% | 34.1% (n=399) | Below target |
| LOW confidence accuracy | 40–50% | 40.0% (n=45) | Met |
| Total predictions | 500+ | 490 | Near target |
| Tickers covered | 10 | 10 | Met |
| Avg inference latency (GPU) | < 20s | ~12s | Met |
| Total backtest time | — | ~90 min | — |

The 33.9% directional accuracy falls below the 50% random baseline for a
three-class problem (UP/DOWN/SIDEWAYS). This result is attributable to two
compounding factors: (1) phi3:mini at 3.8B parameters lacks the reasoning
capacity required for reliable financial analysis, and (2) the synthetic
random-walk data generated by `generate_csv.py` contains no exploitable
patterns — the close prices follow a geometric Brownian motion with
μ=0 and σ=0.15% per bar, making prediction inherently impossible.

The confidence calibration is inverted: LOW confidence predictions (40.0%)
outperform HIGH confidence predictions (26.1%). This indicates that the
model's self-assessed confidence is anti-correlated with actual accuracy —
a known limitation of small language models that lack calibration training.

![Figure 3: Accuracy Over Time](visuals/accuracy_over_time.png)
*Figure 3: Rolling prediction accuracy across the 490-prediction backtest.
The green shaded area indicates above-random performance; red indicates
below-random. Accuracy fluctuates around 30–40% with phi3:mini on synthetic data.*

### 7.3 Measured Latency Profile

![Figure 4: Inference Latency](visuals/latency_profile.png)
*Figure 4: Measured inference latency distribution on Tesla T4 GPU. Median
~12s per full prediction cycle (3 specialist agents at ~3s each + synthesizer
at ~3s). The distribution is tight around the median, indicating consistent
GPU inference performance.*

### 7.4 Confidence Calibration

![Figure 5: Confidence Calibration](visuals/confidence_calibration.png)
*Figure 5: Prediction accuracy by confidence level. The ideal pattern
(HIGH > MED > LOW) is inverted, with LOW confidence predictions (40.0%)
outperforming HIGH confidence (26.1%). This anti-correlation indicates
that phi3:mini's self-assessed confidence is not reliable for risk
management — a critical finding for system deployment.*

### 7.5 Accuracy by Ticker

![Figure 6: Accuracy by Ticker](visuals/accuracy_by_ticker.png)
*Figure 6: Directional accuracy per NSE ticker. Results vary significantly
across tickers, consistent with the random-walk nature of synthetic data
where no exploitable patterns exist.*

### 7.6 Predicted vs Actual Returns

![Figure 7: Predicted vs Actual](visuals/pred_vs_actual.png)
*Figure 7: Scatter plot of predicted vs actual returns. Green dots indicate
correct directional predictions; red dots indicate incorrect. The near-zero
correlation confirms that phi3:mini cannot reliably predict return magnitude
on synthetic data.*
